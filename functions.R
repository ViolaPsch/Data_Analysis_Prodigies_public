plot_hist <- function(variable, x_lab, title, x_intercept, x_intercept_2 = NULL, print_binwidth){
  ...
  x <- ceiling(
    max(DATA_prtcpnt[[variable]], na.rm = TRUE) - 
      min(DATA_prtcpnt[[variable]], na.rm = TRUE)
  ) / 10
  ...
  x_lim_right <- ceiling(max(DATA_prtcpnt[[variable]], na.rm = TRUE)/(binwidth/2))*(binwidth/2)
  
  if(x_lim_right <= max(DATA_prtcpnt[[variable]], na.rm = TRUE) + (binwidth/2)){
    x_lim_right <- x_lim_right + (binwidth/2)
  }
  
  x_lim_left  <- floor(min(DATA_prtcpnt[[variable]], na.rm = TRUE)/(binwidth/2))*(binwidth/2)
  
  if(x_lim_left >= min(DATA_prtcpnt[[variable]], na.rm = TRUE) - (binwidth/2)){
